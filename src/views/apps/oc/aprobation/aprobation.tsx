...
import FormDialog from './dialogs/aprobationDialog'
...
type AprobationTableWithActionsType = aprobationTypeTAble & {
  actions?: string
}
...
// Column Definitions
const columnHelper = createColumnHelper<AprobationTableWithActionsType>()

const AprobationTable = ({ aprobationData }: { aprobationData?: aprobationTypeTAble[] }) => {
  console.log('aqui es', aprobationData)

  // States
  const [rowSelection, setRowSelection] = useState({})
  const [data, setData] = useState(...[aprobationData])
  const [filteredData, setFilteredData] = useState(aprobationData)
  const [globalFilter, setGlobalFilter] = useState('')

  // Hooks
  const { lang: locale } = useParams()

  const columns = useMemo<ColumnDef<AprobationTableWithActionsType, any>[]>(
    () => [
      {
        id: 'select',
        header: ({ table }) => (
      ...
      columnHelper.accessor('provider', {
        header: 'Proveedor',
        cell: ({ row }) => <Typography color='text.primary'>{row.original.provider}</Typography>
      }),
      ...
      columnHelper.accessor('observation', {
        header: 'Observación',
        cell: ({ row }) => <Typography>{row.original.observation}</Typography>
      }),
  ...
  const table = useReactTable({
    data: data as aprobationTypeTAble[],
    columns,
    filterFns: {
      fuzzy: fuzzyFilter
    },
        ...
        <div className='flex items-center justify-between flex-wrap gap-4 p-6'>
          <Grid container spacing={6}>
            ...
            <Grid item xs={12} sm={12}>
              <Grid container spacing={6}>
                <Grid item xs={12} sm={4}>
                  <CustomTextField select sx={{ width: '100%', mr: '10px' }} id='select-status'>
                    <MenuItem value=''>Accion</MenuItem>
                    <MenuItem value='Scheduled'>Scheduled</MenuItem>
                    <MenuItem value='Published'>Publish</MenuItem>
                    <MenuItem value='Inactive'>Inactive</MenuItem>
                  </CustomTextField>
                ...
                <Grid item xs={12} sm={4}>
                  {/* <Button variant='contained'>Ejecutar Acción</Button> */}
                  <FormDialog />
                </Grid>
              </Grid>
            </Grid>
...
export default AprobationTable
