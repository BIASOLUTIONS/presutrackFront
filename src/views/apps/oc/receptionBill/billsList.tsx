...
import SincDialog from './dialogs/sincDialog'
...
type ReceptionBillsTableWithActionsType = receptionBillsTypeTAble & {
  actions?: string
}
...
// Column Definitions
const columnHelper = createColumnHelper<ReceptionBillsTableWithActionsType>()

const ReceptiomBillsTable = ({ billsData }: { billsData?: receptionBillsTypeTAble[] }) => {
  // States
  const [rowSelection, setRowSelection] = useState({})
  const [data, setData] = useState(...[billsData])
  const [filteredData, setFilteredData] = useState(billsData)
  const [globalFilter, setGlobalFilter] = useState('')

  // Hooks
  const { lang: locale } = useParams()

  const columns = useMemo<ColumnDef<ReceptionBillsTableWithActionsType, any>[]>(
    () => [
      ...
      columnHelper.accessor('numberBill', {
        header: 'Numero de Factura',
        cell: ({ row }) => <Typography>{row.original.numberBill}</Typography>
      }),
      columnHelper.accessor('dateBill', {
        header: 'Fecha de factura',
        cell: ({ row }) => <Typography>{row.original.dateBill}</Typography>
      }),
          ...
          <Grid container spacing={6}>
            <Grid item xs={12} sm={4}>
              <CustomTextField select fullWidth id='select-status'>
                <MenuItem value=''>Filtrar por NIT – Razón Social</MenuItem>
                <MenuItem value='900111222 – EMPRESA SAS'>900111222 – EMPRESA SAS</MenuItem>
              </CustomTextField>
            </Grid>
            <Grid item xs={12} sm={4}>
              <CustomTextField select fullWidth id='select-status'>
                <MenuItem value=''>Centro de Costo</MenuItem>
...
export default ReceptiomBillsTable
